...
print("DEBUG: parent folder files =", os.listdir(os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))))

from v2_multilabel_rulebased_sgd import (
    hybrid_predict,
    load_stopwords,
    setup_nltk,
    load_lemma_dict,
...
st.title('SGD-Hybrid Model Multilabel Hate Speech Detection')
...
if st.button('Predict'):
    if user_input == "":
        ...
        with open('pages/sgd_model.pkl', 'rb') as model_file:
            model_bundle = pickle.load(model_file)

        model = model_bundle['sgd_model']
